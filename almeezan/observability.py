"""Structured logging and in-process metrics for Almeezan.

Logging writes JSON lines (one event per line) so logs can be shipped to any
log pipeline without parsing free text. Metrics are process-local, thread-safe
counters plus a bounded latency reservoir exposed via ``/api/metrics``.
"""
...
def setup_logging(name: str, log_dir: Path, level: str = "INFO") -> logging.Logger:
    ...
    if getattr(logger, "_almeezan_configured", False):
        return logger
    ...
    logger._almeezan_configured = True  # type: ignore[attr-defined]
    return logger
